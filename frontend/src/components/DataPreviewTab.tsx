        ...
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-gh-surface">
            <tr>
              {cols.map((c) => (
                <th
                  key={c}
                  className="text-left px-3 py-2.5 text-gh-muted font-semibold border-b border-gh-border whitespace-nowrap"
                >
                  {c}
                </th>
              ))}
          ...
          <tbody>
            {displayed.map((row, i) => (
              <tr
                key={i}
                className="border-b border-gh-border hover:bg-gh-surface transition-colors"
              >
                {cols.map((c) => (
